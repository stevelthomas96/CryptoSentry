...
# ---------------- PLACEHOLDER TABS ---------------- #
   ...
   # --- Load actual data files ---
    ...
    st.markdown(
        "This portfolio is generated by a sentiment-enhanced optimization model, which adjusts allocations "
        "based on each token's recent news sentiment and price volatility."
    )

    # Load mock portfolio
    sentiment_portfolio_df = pd.read_csv("data_outputs/sentiment_portfolio.csv")

    # Format and display
    sentiment_portfolio_df["Weight (%)"] = sentiment_portfolio_df["Weight (%)"].round(2)
    st.dataframe(sentiment_portfolio_df, use_container_width=True, hide_index=True)
    ...
    fig = px.bar(
        ...
        y="Weight (%)",
        color="Token",
        title="📊 Token Allocation (Sentiment-Based)",
